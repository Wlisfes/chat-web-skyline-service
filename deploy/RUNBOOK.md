...
`chat-web-skyline-service.yaml` 的相关配置如下。Skyline 只调用 Finance，目标服务地址单独配置：
...
Skyline 只读取 `feign.chat-web-finance.url/timeout`；缺少任一字段时部署校验会中止。不要在 Skyline Nacos 配置未使用的目标服务节点。
...
流水线会先把 `deploy/bootstrap-nacos-config.cjs` 安装到 `/opt/chat-web-skyline-service`，再使用 `node:22-alpine`（加入 `chat-web-infrastructure` 网络）执行只读校验。脚本不会回写 Nacos，不会补齐或覆盖任何业务配置；它校验 `server.port: 5040`、`database.chat-web-skyline` 以及 `feign.service_token`、`feign.chat-web-finance.url/timeout`。脚本不会把 Nacos 配置正文或凭据写入 Runner 日志；缺少节点/凭据时应先人工配置后重跑流水线。

每日汇率任务通过独立 Finance Feign 地址调用 `/feign/finance/currency/exchange/sync`，Skyline 只触发一次并原样返回 Finance 结果。自动调度和手动触发都使用 Nacos `feign.service_token` 服务凭据，不转发用户 Bearer 令牌。令牌不得写入仓库或日志。
