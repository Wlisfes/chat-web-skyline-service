...
`chat-web-skyline-service.yaml` 的相关配置如下。Skyline 的 Feign 请求统一经 Gateway 转发：
...
Skyline 只读取 `gateway.feign.url/timeout`；缺少任一字段时部署校验会中止。所有 Feign 请求都通过 Gateway 的服务路由转发。
