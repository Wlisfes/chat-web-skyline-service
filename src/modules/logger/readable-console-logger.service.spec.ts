...
describe('ReadableConsoleLogger', () => {
    ...
    it('启用颜色时为请求头和 JSON 字段输出不同颜色', () => {
        ...
        expect(lines[0]).toContain('\u001B[38;2;255;61;104m执行方法:[LoggerMiddleware]\u001B[39m')
        expect(lines[0]).toContain('\u001B[38;2;252;84;4m接口地址:[/]\u001B[39m')
        expect(lines[0]).toContain('\u001B[38;2;255;61;104m耗时:3ms\u001B[39m')
    ...
    it('生产环境将请求输出为 Dozzle 可展开的单行 JSON', () => {
        ...
        expect(record.message).toContain('服务名称:[chat-web-skyline-service]')
        expect(record.message).toContain('执行方法:[LoggerMiddleware]')
        expect(record.message).toContain('日志ID:[dozzle-request]')
        expect(record.message).toContain('接口地址:[/]')
        expect(record.message).toContain('耗时:3ms')
        expect(details.service).toBe('chat-web-skyline-service')
        expect(details.logId).toBe('dozzle-request')
