...
- 汇率同步任务通过 `FeignClientFinanceManager` 调用 Finance 服务 `/feign/finance/currency/exchange/sync`，外部汇率数据从 Frankfurter 获取。Feign 客户端统一复用 `chat-web-base-schema`，请求经 Gateway 转发并始终使用 Nacos `feign.service_token`，不得把用户 JWT 传给 Finance。
